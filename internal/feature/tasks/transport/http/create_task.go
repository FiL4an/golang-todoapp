...
type CreateTaskRequest struct {
	Title        string  `json:"title" validate:"required,min=1,max=100" example:"Побегать с собакой"`
	Description  *string `json:"description" validate:"omitempty,min=1,max=1000" example:"Пробежка с собакой в 9 утра"`
...
// @Description Создание задачи для конкретного пользователя
// @Tags tasks
// @Accept json
// @Produce json
...
func (h *TasksHTTPHandler) CreateTask(w http.ResponseWriter, r *http.Request) {
	...
	taskDomain := domain.NewTaskUnintialized(request.Title, request.Description, request.AuthorUserID)
	if err := taskDomain.Validate(); err != nil {
		responseHandler.ErrorResponse(err, "failed to validate task")
		return
	}
