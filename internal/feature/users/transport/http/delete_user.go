...
// @Description Удаление сущесвующего в системе пользователя
// @Tags users
// @Param id path int true "ID удаляемого пользователя"
// @Success 204 "Усешное удаление пользователя"
// @Failure 400 {object}  core_http_response.ErrorResponse "Bad request"
// @Failure 404  {object} core_http_response.ErrorResponse "User not found"
// @Failure 500 {object}  core_http_response.ErrorResponse "Internal server error"
// @Router /users/{id} [delete]
func (s *UsersHTTPHandler) DeleteUser(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	if err := s.usersService.DeleteUser(ctx, userID); err != nil {
		responseHandler.ErrorResponse(err,
			"failed delete user")
		return
	}
